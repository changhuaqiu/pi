...
import type { LogosApprovalSubject } from "./logos-tools.ts";
import type {
	ToolPermission,
	ToolPolicyInfo,
} from "./tool-system.ts";
...
function wrapPlainLine(value: string, width: number): string[] {
	...
	let startColumn = 0;
	while (startColumn < totalWidth) {
		const chunk = sliceByColumn(normalized, startColumn, width);
		if (!chunk) break;
		rows.push(chunk);
		startColumn += visibleWidth(chunk);
	}
	return rows;
}
...
function formatImpact(subject: Extract<LogosApprovalSubject, { kind: "edit" }>): string | undefined {
	const impact = subject.impact;
	if (!impact) return undefined;
	if (impact.status === "unavailable") {
		return ` Impact unavailable: ${sanitizeTerminalText(impact.reason ?? "no reliable symbol match")}`;
	}
	const locations = impact.affected.map((anchor) =>
		anchor.file === undefined
			? anchor.name
			: `${anchor.file}${anchor.line === undefined ? "" : `:${anchor.line}`}`
	);
	const suffix = locations.length === 0 ? "" : `: ${locations.join(", ")}`;
	return ` Impact: ${impact.symbol?.name ?? "symbol"} affects ${impact.totalAffected} indexed symbol${impact.totalAffected === 1 ? "" : "s"}${suffix}`;
}

export class LogosApprovalCard implements Component {
	private readonly subject: LogosApprovalSubject;
	private readonly diffLines: string[];
	private renderedDiffLines: string[];
	private scrollOffset = 0;

	constructor(subject: LogosApprovalSubject) {
	...
	render(width: number): string[] {
		...
		const lines = [
			boundedLine(chalk.bold.yellow("Review before Logos Agent acts"), safeWidth),
		];
		...
		if (this.subject.kind === "edit") {
			...
			const impact = formatImpact(this.subject);
			if (impact) {
				lines.push(...wrapPlainLine(impact, contentWidth).map((line) => ` ${line}`));
			}
			lines.push(chalk.dim(boundedLine(` ${"─".repeat(Math.max(1, contentWidth))}`, safeWidth)));

			const diffStartRow = lines.length;
			...
			while (lines.length < diffStartRow + approvalViewportRows) {
				lines.push("");
			}
			const rangeStart =
		...
		} else if (this.subject.kind === "directories") {
			lines.push(
				boundedLine(
					` Create ${this.subject.paths.length} workspace director${this.subject.paths.length === 1 ? "y" : "ies"} recursively:`,
					safeWidth,
				),
			);
			this.renderedDiffLines = this.subject.paths.flatMap((path) =>
				wrapPlainLine(`- ${path}`, contentWidth),
			);
			this.scrollOffset = this.clampOffset(this.scrollOffset);
			const end = Math.min(
				this.scrollOffset + approvalViewportRows,
			...
			lines.push(
				chalk.dim(
					boundedLine(
						" Only directories are created; no shell command will run.",
						safeWidth,
					),
				),
			);
		} else if (this.subject.kind === "command") {
			const command = this.subject.command;
			lines.push(
				boundedLine(
					` ${command.operation === "npm_install" ? "Dependency install" : "Package script"}`,
					safeWidth,
				),
				boundedLine(
					` Cwd: ${sanitizeTerminalText(command.cwd)}  Mode: ${command.mode}`,
					safeWidth,
				),
				boundedLine(
					` Total limit: ${command.timeoutMs}ms (${(command.timeoutMs / 1000).toFixed(3)}s)`,
					safeWidth,
				),
			);
			if (command.mode === "service") {
				lines.push(
					boundedLine(
						` Startup observation: ${command.startupWaitMs}ms (${(command.startupWaitMs / 1000).toFixed(3)}s)`,
						safeWidth,
					),
				);
			}
			this.renderedDiffLines = [
				...
				...(command.scripts ?? []).map(
					(script) =>
						`package.json ${script.name}: ${script.command}`,
				),
				...command.risks.map((risk) => `risk: ${risk}`),
			].flatMap((line) => wrapPlainLine(line, contentWidth));
			this.scrollOffset = this.clampOffset(this.scrollOffset);
			const end = Math.min(
				this.scrollOffset + approvalViewportRows,
				this.renderedDiffLines.length,
			...
			lines.push(
				chalk.yellow(
					boundedLine(
						" Not sandboxed: approved project code can access files and the network.",
						safeWidth,
					),
				),
			);
		} else if (this.subject.kind === "process_stop") {
			const process = this.subject.process;
			lines.push(
				boundedLine(
					` Stop: ${sanitizeTerminalText(process.command)}`,
					safeWidth,
				),
				boundedLine(
					` Process: ${sanitizeTerminalText(process.processId)}  Status: ${process.status}`,
					safeWidth,
				),
				boundedLine(
					` Cwd: ${sanitizeTerminalText(process.cwd)}`,
					safeWidth,
				),
				chalk.dim(
					boundedLine(
						" The managed process and its descendants will be terminated.",
						safeWidth,
					),
				),
			);
...
export class LogosSessionPicker implements Component, Focusable {
	private readonly sessions: readonly LogosAgentSessionListItem[];
	private readonly currentSessionId: string;
	private readonly searchInput = new Input();
	private filtered: LogosAgentSessionListItem[];
	private selectedIndex = 0;
	private _focused = false;

	onSelect?: (session: LogosAgentSessionListItem) => void;
	onCancel?: () => void;

	constructor(
		sessions: readonly LogosAgentSessionListItem[],
	...
	render(width: number): string[] {
		const safeWidth = Math.max(1, width);
		const lines = [
			boundedLine(chalk.bold.cyan("Resume a logos session"), safeWidth),
...
export class LogosToolPolicyPicker implements Component {
	...
	render(width: number): string[] {
		const safeWidth = Math.max(1, width);
		return [
			boundedLine(chalk.bold.cyan("Logos Agent tool policy"), safeWidth),
			boundedLine(
