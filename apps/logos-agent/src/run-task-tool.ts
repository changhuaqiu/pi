...
export type RunTaskName = "logos_agent_test" | "logos_agent_typecheck";
...
const taskSummaries: Record<RunTaskName, RunTaskApprovalSummary> = {
	logos_agent_test: {
		task: "logos_agent_test",
		label: "Logos Agent tests",
		command: "node --import tsx --test test/*.test.ts",
		cwd: "apps/logos-agent",
		timeoutMs: taskTimeoutMs,
	},
	logos_agent_typecheck: {
		task: "logos_agent_typecheck",
		label: "Logos Agent typecheck",
		command:
			"node node_modules/typescript/bin/tsc --noEmit -p apps/logos-agent/tsconfig.json",
		cwd: ".",
		timeoutMs: taskTimeoutMs,
	},
};
...
export function createNodeRunTaskOperations(
	...
	return {
		async run(task, signal) {
			...
			const appRoot = await realpath(join(resolvedRoot, "apps", "logos-agent"));
			if (!isWithinRoot(resolvedRoot, appRoot)) {
				throw new Error("Logos Agent task cwd escapes the workspace");
			}
			let command: string;
			let args: string[];
			let cwd: string;
			if (task === "logos_agent_test") {
				const testRoot = await realpath(join(appRoot, "test"));
				if (!isWithinRoot(appRoot, testRoot)) {
					throw new Error("Logos Agent test directory escapes the application root");
				}
				...
				if (!isWithinRoot(resolvedRoot, tsxPath)) {
					throw new Error("Logos Agent test loader escapes the workspace");
				}
				...
				if (!isWithinRoot(resolvedRoot, tscPath) || !isWithinRoot(appRoot, tsconfigPath)) {
					throw new Error("Logos Agent typecheck files escape the workspace");
				}
				command = process.execPath;
				args = [
					tscPath,
