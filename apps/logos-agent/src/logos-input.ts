...
export interface LogosInputContext {
	text: string;
...
export interface LogosInputDescriptor {
	mode: LogosInputMode;
	label: string;
	help: string;
}
...
export function describeLogosInput(
	context: LogosInputContext,
): LogosInputDescriptor {
	const text = context.text;
	if (context.awaitingUserAnswer) {
		return {
			mode: "answer",
			label: "ANSWER",
			help:
				context.questionOptionCount > 0
					? `choose 1-${context.questionOptionCount}, type something, or return to chat · Enter select · Esc cancel turn`
					: "type something or return to chat · Enter select · Esc cancel turn",
		};
	}
