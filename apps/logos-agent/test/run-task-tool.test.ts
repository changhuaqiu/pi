...
test("run_task exposes only fixed validation tasks and reports progress", async () => {
	...
	assert.throws(
		() => parseRunTaskInput({ task: "logos_agent_test", cmd: "rm -rf ." }),
		/execution-time validation/,
	);
	assert.equal(describeRunTask("logos_agent_typecheck").cwd, ".");
});
...
test("node run_task adapter executes the fixed Logos Agent typecheck", async () => {
...
test("node run_task adapter rejects promptly when tree termination fails", async () => {
	...
	const task = operations.run("logos_agent_typecheck", controller.signal);
	setTimeout(() => controller.abort(), 25);
...
test(
	...
	{ skip: process.env.LOGOS_AGENT_FIXED_TASK === "1" },
	async () => {
		const workspaceRoot = resolve(import.meta.dirname, "..", "..", "..");
