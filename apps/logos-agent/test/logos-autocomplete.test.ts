...
import { LogosAutocompleteProvider } from "../src/logos-autocomplete.ts";

async function suggestions(
	provider: LogosAutocompleteProvider,
	text: string,
): Promise<string[]> {
...
test("workspace autocomplete suggests safe files and rejects path escapes", async () => {
	const tempRoot = await mkdtemp(join(tmpdir(), "logos-autocomplete-"));
	...
	try {
		const provider = new LogosAutocompleteProvider([], workspace);
