...
八月中旬，我又打开了一条完整开发 Trace。

这次用户问的是：Agent 评测完回答以后，能不能把追问更新进面试题库。Agent 没有像早期链路那样把一句模糊请求直接变成代码。它先解释三种入库方式，再停下来等待用户选择。用户确认“追问一键入库”后，它才进入实现。

后面的路线也比第一次排演清楚：读取源码、记录 Plan、修改评测返回和前端闭环、运行语法检查、启动真实服务、请求真实评测接口、记录 Reflection，最后调用 Finish。
...
| Span | 196 | 71 | 降低约 64% |
| 明确错误 | 8 | 2 | 降低 75% |
| 新输入与输出 Token | 105,569 | 77,956 | 降低约 26% |

模糊请求会先确认，工具试错明显减少，修改后会运行真实测试，任务也能经过 `reflect_task` 和 `finish_task` 稳定退出。此前加入的工具 Interface、失败恢复、测试规范和完成协议都产生了效果。

但我把 Token 口径切换到 `cacheRead + fresh input + output` 后，故事突然变了。

```text
八月初累计处理：13,134,945 Token
当前累计处理：  12,636,036 Token
```

请求数量下降了 64%，实际处理 Token 却只下降 3.8%。Provider Payload 总量也只从约 48.39 MB 降到 45.68 MB。

第一次故障是“很多请求乘以中等 Context”。当前故障变成了“较少请求乘以巨大 Context”。当前模型窗口有 100 万 Token，首轮输入已经带着约 342,000 Token，最后增长到约 371,000 Token。它没有接近溢出，却在 35 次 Provider 请求中累计重复读取 12,558,080 个缓存 Token。

缓存把 Trace 成本保持在约 `$0.0871`，却没有让背包消失。除去等待用户选择，Agent 实际执行约 358 秒，其中约 321 秒都在等待 Provider。35 次请求累计发送了约 45.7 MB Payload。

更值得警惕的是证据新鲜度。这个 Turn 只重新读取了 `app.js`，却修改了六个文件。端口冲突后，Agent还把一个临时测试环境问题变成了永久产品修改：新增 `kill-port.mjs` 和 `npm run killport`。这不能只凭一条 Trace 证明由大 Context 直接导致，却说明旧设计、旧文件内容和当前目标已经开始争夺模型注意力。

因此，故事的下一阶段不再是继续增加工具。我们需要把完整 Session 和 Provider 工作集真正分开：Session 保存所有事实和审计；Provider 只拿当前任务的 Task Checkpoint、最近原文和仍然有效的证据。工具结果需要生命周期，Proposal 应在 Apply 后压缩，旧 Read 应在文件修改后失效，成功测试应收敛为结构化证据。

完整的方法、量化基线、目标架构、Soft/Hard Budget、Tool Result 生命周期和验收顺序记录在 [`context-governance.md`](./context-governance.md)。它把下一阶段的目标从“70% 时避免窗口溢出”改成了“在整个 Agent Loop 中控制信息密度和累计处理量”。
