...
public static class LearningAgentJob {
...
$job = [LearningAgentJob]::CreateJobObject([IntPtr]::Zero, $null)
...
$exitCode = 1
try {
	$limit = New-Object LearningAgentJob+ExtendedLimitInformation
	...
	try {
		...
		if (-not [LearningAgentJob]::SetInformationJobObject($job, 9, $pointer, $length)) {
			throw [ComponentModel.Win32Exception]::new([Runtime.InteropServices.Marshal]::GetLastWin32Error())
		}
	}
	finally {
	...
	if (-not [LearningAgentJob]::AssignProcessToJobObject($job, $currentProcess.Handle)) {
		throw [ComponentModel.Win32Exception]::new([Runtime.InteropServices.Marshal]::GetLastWin32Error())
	}
	...
	if (-not $wrapperAssigned) {
		[void] [LearningAgentJob]::CloseHandle($job)
	}
}
