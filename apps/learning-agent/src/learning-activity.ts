...
const phaseVerbs: Record<Exclude<LearningPhase, "idle">, readonly string[]> = {
	...
	compacting: ["Compressing context", "Preserving learning state"],
	session: ["Loading learning session", "Rebuilding context"],
};
...
export interface LearningActivityDescription {
	frame: string;
	verb: string;
	timing: string;
	secondary: string;
...
function timingLabel(phase: Exclude<LearningPhase, "idle">): string {
...
export function describeLearningActivity(
	snapshot: LearningProgressSnapshot,
): LearningActivityDescription | undefined {
	...
	const tip =
		learningTips[Math.floor(elapsedMs / tipDurationMs) % learningTips.length] ??
		learningTips[0];
...
export class LearningActivityIndicator implements Component {
	private description?: LearningActivityDescription;

	update(snapshot: LearningProgressSnapshot): boolean {
		const next = describeLearningActivity(snapshot);
		if (
